...
class CIRISClient:
    ...
    def __init__(self, base_url: str = "http://localhost:8080", username: str = "admin", password: str = "ciris_admin_password"):
    ...
    def get_audit_entries(self, limit: int = 10, handler: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            response = self.session.get(
                f"{self.base_url}/v1/audit/entries",
                params=params
            )
            response.raise_for_status()
            ...
            if handler:
                ...
                for entry in entries:
                    ...
                    if action == f"AUDITEVENTTYPE.HANDLER_ACTION_{handler.upper()}" or \
                       action == f"AUDITEVENTTYPE.HANDLER_{handler.upper()}":
