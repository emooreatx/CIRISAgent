...
Actions chosen by the PDMA are routed through an `ActionDispatcher`. Memory operations use `CIRISLocalGraph` (formerly `DiscordGraphMemory`) for persistence.
...
*   **Graph Memory:** MEMORIZE actions store user metadata in `CIRISLocalGraph` (previously `DiscordGraphMemory`). REMEMBER and FORGET exist but are often disabled via profiles during testing.
