...
class CIRISAPIClient:
    ...
    def get_memory(self, node_id: str) -> Dict[str, Any]:
        ...
        resp = requests.get(f"{self.base_url}/v1/memory/{node_id}", headers=self.headers)
        return resp.json() if resp.status_code == 200 else {"error": resp.text}

    def search_memory(self, query: str) -> List[Dict[str, Any]]:
        """Search memory nodes."""
        resp = requests.get(f"{self.base_url}/v1/memory/search", params={"q": query}, headers=self.headers)
        if resp.status_code == 200:
            result = resp.json()
    ...
    def get_tasks(self, status: str = None) -> List[Dict[str, Any]]:
        ...
        resp = requests.get(f"{self.base_url}/v1/agent/tasks", params=params, headers=self.headers)
        if resp.status_code == 200:
            result = resp.json()
...
class TestHandlers:
    ...
    def test_recall(self, api_client):
        ...
        if "still processing" not in response.lower():
            ...
            assert any(
                ...
                for word in ["found", "recall", "memory", "no memories", "mock llm", "mockllm", "speak in response"]
            )
        else:
    ...
    def test_observe(self, api_client):
        ...
        if "Still processing" in response:
            ...
            # Accept various mock LLM responses
            assert any(
                word in response.lower() for word in ["observ", "channel", "mock llm", "mockllm", "speak in response"]
            )
    ...
    def test_reject(self, api_client):
        ...
        if "Still processing" in response:
            ...
            assert any(
                word in response.lower()
                for word in ["reject", "inappropriate", "mock llm", "mockllm", "speak in response"]
            )
