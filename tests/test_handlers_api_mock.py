...
class CIRISAPIClient:
    ...
    def login(self, username: str = "admin", password: str = "ciris_admin_password") -> bool:
        ...
        resp = requests.post(f"{self.base_url}/v1/auth/login", json={"username": username, "password": password})
    ...
    def get_audit_entries(self, limit: int = 10, resource_filter: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        resp = requests.get(f"{self.base_url}/v1/audit/entries?limit={limit}", headers=self.headers)
        if resp.status_code == 200:
    ...
    def search_memory(self, query: str) -> List[Dict[str, Any]]:
        """Search memory graph."""
        resp = requests.post(f"{self.base_url}/v1/memory/search", json={"query": query}, headers=self.headers)
        if resp.status_code == 200:
    ...
    def get_traces(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        resp = requests.get(f"{self.base_url}/v1/telemetry/traces?limit={limit}", headers=self.headers)
        if resp.status_code == 200:
    ...
    def find_handler_action(self, handler_name: str, limit: int = 50, time_window_seconds: int = 20) -> bool:
        ...
        for entry in entries:
            ...
            # Check handler match
            if (
                handler_name in str(entry.get("actor", ""))
                or handler_name.upper() in str(entry.get("action", ""))
                or f"HANDLER_ACTION_{handler_name.upper().replace('HANDLER', '')}" in str(entry.get("action", ""))
            ):
                return True
...
class TestHandlerIntegration:
    ...
    def test_memorize_and_recall_flow(self, api_client):
        ...
        entries = api_client.get_audit_entries(limit=50)
        handler_entries = [
            e for e in entries if "Handler" in e.get("actor", "") or "HANDLER" in str(e.get("action", ""))
        ]
    ...
    def test_ponder_and_speak_flow(self, api_client):
        ...
        ponder_found = any(
            "PonderHandler" in e.get("actor", "") or "AuditEventType.HANDLER_ACTION_PONDER" in str(e.get("action", ""))
            for e in entries
        )

        # Look for SpeakHandler or HANDLER_ACTION_SPEAK
        speak_found = any(
            "SpeakHandler" in e.get("actor", "") or "AuditEventType.HANDLER_ACTION_SPEAK" in str(e.get("action", ""))
            for e in entries
        )

        assert ponder_found, "PONDER handler not found in audit entries"
        assert speak_found, "SPEAK handler not found in audit entries"
