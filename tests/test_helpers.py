...
def create_test_task_with_persistence(
    task_id: Optional[str] = None, channel_id: str = "test_channel", description: str = "Test task"
) -> Task:
    ...
    task = Task(
        ...
        context=TaskContext(
            channel_id=channel_id, user_id="test_user", correlation_id=f"test_correlation_{uuid.uuid4().hex[:8]}"
        ),
    )
...
def create_test_thought_with_persistence(
    ...
    thought = Thought(
        ...
        context=ThoughtContext(
            task_id=task_id, round_number=1, depth=1, correlation_id=f"test_correlation_{uuid.uuid4().hex[:8]}"
        ),
    )
