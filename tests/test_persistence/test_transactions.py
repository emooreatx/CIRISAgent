    ...
    def test_concurrent_updates_same_row(self, temp_db_path: str):
        ...
        def increment_priority(thread_id: int) -> TransactionResult:
            ...
            return TransactionResult(
                success=False, error="Max retries exceeded", duration_ms=duration_ms, retry_count=retry_count
            )
    ...
    def test_potential_deadlock_scenario(self, temp_db_path: str):
        ...
        def transaction1():
            try:
                ...
                # Update task A
                cursor.execute("UPDATE tasks SET status = ? WHERE task_id = ?", (TaskStatus.ACTIVE.value, "task_A"))
                ...
                cursor.execute("UPDATE tasks SET status = ? WHERE task_id = ?", (TaskStatus.ACTIVE.value, "task_B"))
        ...
        def transaction2():
            ...
            try:
                ...
                cursor.execute("UPDATE tasks SET status = ? WHERE task_id = ?", (TaskStatus.COMPLETED.value, "task_B"))
