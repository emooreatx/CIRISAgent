    ...
    def test_register_simple_node(self):
        ...
        class SimpleNode(TypedGraphNode):
            ...
            def from_graph_node(cls, node: GraphNode) -> "SimpleNode":
                ...
                return cls(id=node.id, type=node.type, scope=node.scope, test_field=attrs.get("test_field", "test"))
    ...
    def test_register_multiple_nodes(self):
        ...
        for node_type in node_types:
            ...
            def make_to_graph_node(nt):
                def to_graph_node(self) -> GraphNode:
                    return GraphNode(id=self.id or nt.lower(), type=nt, scope=self.scope, attributes={})
    ...
    def test_manual_registration(self):
        ...
        class ManualNode(TypedGraphNode):
            ...
            def from_graph_node(cls, node: GraphNode) -> "ManualNode":
                attrs = node.attributes if isinstance(node.attributes, dict) else {}
                return cls(id=node.id, type=node.type, scope=node.scope, manual_field=attrs.get("manual_field", 42))
...
class TestComplexNodeTypes:
    ...
    def test_nested_data_structures(self):
        ...
        class NestedNode(TypedGraphNode):
            ...
            def to_graph_node(self) -> GraphNode:
                attrs = {"data": self.data, "node_class": "NestedNode"}
                if self.metadata:
    ...
    def test_inheritance_hierarchy(self):
        ...
        class UserEventNode(BaseEventNode):
            ...
            def to_graph_node(self) -> GraphNode:
                ...
                attrs.update({"user_id": self.user_id, "action": self.action, "node_class": "UserEventNode"})
            ...
            def make_class(nt, idx):
                class DynamicNode(TypedGraphNode):
                    ...
                    def from_graph_node(cls, node: GraphNode) -> "DynamicNode":
                        attrs = node.attributes if isinstance(node.attributes, dict) else {}
                        return cls(id=node.id, type=node.type, scope=node.scope, index=attrs.get("index", idx))
        ...
        class PerfTestNode(TypedGraphNode):
            ...
            def to_graph_node(self) -> GraphNode:
                return GraphNode(
                    ...
                    type=NodeType.OBSERVATION,
                    scope=self.scope,
                    attributes={"counter": self.counter, "data": self.data, "node_class": "PerfTestNode"},
                )
...
class TestRegistryEdgeCases:
    ...
    def test_empty_registry(self):
        ...
        graph_node = GraphNode(id="test", type=NodeType.CONCEPT, scope=GraphScope.LOCAL, attributes={})
    ...
    def test_register_with_special_characters(self):
        ...
        for special_type in special_types:
            ...
            class SpecialNode(TypedGraphNode):
                def to_graph_node(self) -> GraphNode:
                    return GraphNode(id="special", type=special_type, scope=GraphScope.LOCAL, attributes={})
