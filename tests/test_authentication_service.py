...
class TestAuthenticationServiceUnit:
    ...
    def test_actions_list(self, auth_service):
        ...
        expected_actions = [
            "authenticate", "create_token", "verify_token", "verify_token_sync",
            "create_channel_token", "create_wa", "get_wa", "update_wa", 
            "revoke_wa", "list_was", "rotate_keys", "bootstrap_if_needed",
            "update_last_login", "sign_task", "verify_task_signature",
            "generate_keypair", "sign_data", "hash_password"
        ]
    ...
    async def test_database_initialization(self, auth_service):
        ...
        with sqlite3.connect(auth_service.db_path) as conn:
            cursor = conn.execute(
                "SELECT name FROM sqlite_master WHERE type='table' AND name='wa_cert'"
            )
...
class TestAuthenticationServiceJWT:
    ...
    async def test_channel_token_creation(self, auth_service):
        ...
        long_token = await auth_service.create_channel_token(
            observer.wa_id,
            "discord_general",
            ttl=0
        )
...
class TestAuthenticationServiceIntegration:
    ...
    async def test_adapter_observer_flow(self, auth_service):
        ...
        adapter_info = {
            ...
            "user_id": "api_user",
            "username": "API Observer"
        }
        
        # Create channel token for adapter
        token = await auth_service._create_channel_token_for_adapter(
            adapter_type,
            adapter_info
        )
...
class TestAuthenticationServiceErrorHandling:
    ...
    async def test_update_nonexistent_wa(self, auth_service):
        ...
        result = await auth_service.update_wa(
            "wa-9999-99-99-NONE01",
            updates=WAUpdate(name="New Name")
        )
        assert result is None
    ...
    async def test_create_channel_token_requires_channel(self, auth_service):
        ...
        wa = await auth_service.create_wa(
            "Test User", "test@test.com", ["read"], WARole.OBSERVER
        )
    ...
    async def test_sign_task_no_private_key(self, auth_service):
        ...
        wa = await auth_service.create_wa(
            "Regular User", "user@test.com", ["read"], WARole.OBSERVER
        )
...
class TestAuthenticationServicePerformance:
    ...
    async def test_concurrent_token_verification(self, auth_service):
        ...
        # Create test WA
        wa = await auth_service.create_wa(
            "Concurrent Test", "concurrent@test.com", ["read"], WARole.OBSERVER
        )
        
        # Create multiple tokens
        tokens = [
            auth_service.create_gateway_token(wa, expires_hours=1)
            for _ in range(20)
        ]
    ...
    async def test_token_cache_performance(self, auth_service):
        ...
        wa = await auth_service.create_wa(
            "Cache Test", "cache@test.com", ["read"], WARole.OBSERVER
        )
        token = auth_service.create_gateway_token(wa)
...
class TestAuthenticationServiceSecurity:
    ...
    async def test_scope_enforcement(self, auth_service):
        ...
        wa = await auth_service.create_wa(
            "Limited User", "limited@test.com", 
            ["read:self"],  # Only read:self
            WARole.OBSERVER
        )
