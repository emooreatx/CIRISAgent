...
@pytest.mark.asyncio
async def test_discord_deferral_sink_send_deferral():
    ...
    channel.send.return_value = AsyncMock(id=123)
...
@pytest.mark.asyncio
async def test_discord_deferral_sink_send_deferral_no_channel():
    mock_adapter = MagicMock(spec=DiscordAdapter)
    mock_client = MagicMock()
    mock_adapter.client = mock_client
