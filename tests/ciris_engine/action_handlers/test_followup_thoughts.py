...
async def test_speak_handler_creates_followup():
    ...
    thought = Thought(thought_id="t1", source_task_id="parent1", context={}, status="PENDING", created_at="now", updated_at="now", round_number=1)
...
async def test_remember_handler_creates_followup():
    ...
    deps.memory_service.remember.return_value = MagicMock(status="OK", data="result")
    deps.persistence = MagicMock()
    handler = RememberHandler(deps)
    thought = Thought(thought_id="t2", source_task_id="parent2", context={}, status="PENDING", created_at="now", updated_at="now", round_number=1)
...
async def test_forget_handler_creates_followup():
    deps = MagicMock()
    deps.memory_service = AsyncMock()
    deps.memory_service.forget.return_value = MagicMock(status="OK")
    deps.persistence = MagicMock()
    handler = ForgetHandler(deps)
    thought = Thought(thought_id="t3", source_task_id="parent3", context={}, status="PENDING", created_at="now", updated_at="now", round_number=1)
...
async def test_memorize_handler_creates_followup():
    deps = MagicMock()
    deps.memory_service = AsyncMock()
    deps.memory_service.memorize.return_value = MagicMock(status="SAVED")
    deps.persistence = MagicMock()
    handler = MemorizeHandler(deps)
    thought = Thought(thought_id="t4", source_task_id="parent4", context={}, status="PENDING", created_at="now", updated_at="now", round_number=1)
...
async def test_ponder_handler_creates_followup():
    ...
    thought = Thought(thought_id="t5", source_task_id="parent5", context={}, status="PENDING", created_at="now", updated_at="now", round_number=1, ponder_count=0)
    ...
    # Simulate update_thought_status returns True
...
async def test_task_complete_handler_no_followup():
    ...
    thought = Thought(thought_id="t6", source_task_id="parent6", context={}, status="PENDING", created_at="now", updated_at="now", round_number=1)
    result = ActionSelectionResult(selected_action=HandlerActionType.TASK_COMPLETE, action_parameters={}, rationale="r")
    await handler.handle(result, thought, {"channel_id": "c1"})
    # Should not call add_thought for follow-up
