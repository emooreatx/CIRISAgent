...
class TestTaskManager:
    ...
    def test_activate_pending_tasks_success(
        self, mock_count_active, mock_get_pending, mock_update_status, 
        task_manager, sample_task
    ):
    ...
    @patch('ciris_engine.logic.persistence.count_active_tasks')
    def test_activate_pending_tasks_at_limit(
        self, mock_count_active, task_manager
    ):
    ...
    def test_complete_task_success(
        self, mock_get_task, mock_update_status, task_manager, sample_task
    ):
        ...
        mock_update_status.assert_called_once_with(
            "test-task-123", TaskStatus.COMPLETED, task_manager.time_service
        )
    ...
    def test_fail_task_success(
        ...
        assert result is True
        mock_update_status.assert_called_once_with(
            "test-task-123", TaskStatus.FAILED, task_manager.time_service
        )
    ...
    def test_create_wakeup_sequence_tasks(
        self, mock_uuid, mock_get_env, mock_task_exists, 
        mock_update_status, mock_add_task, task_manager
    ):
        ...
        mock_uuid.side_effect = ["root-corr", "uuid1", "uuid1-corr", "uuid2", "uuid2-corr", 
                                 "uuid3", "uuid3-corr", "uuid4", "uuid4-corr", "uuid5", "uuid5-corr"]
    ...
    def test_create_wakeup_sequence_tasks_existing_root(
        self, mock_get_env, mock_task_exists, mock_update_status, 
        mock_add_task, task_manager
    ):
        ...
        mock_update_status.assert_called_once_with(
            "WAKEUP_ROOT", TaskStatus.ACTIVE, task_manager.time_service
        )
    ...
    def test_cleanup_old_completed_tasks(
        self, mock_get_old, mock_delete, task_manager, sample_task
    ):
    ...
    @patch('ciris_engine.logic.persistence.get_tasks_older_than')
    def test_cleanup_old_completed_tasks_none_found(
        self, mock_get_old, task_manager
    ):
...
class TestEdgeCases:
    ...
    @patch('ciris_engine.logic.persistence.add_task')
    @patch('uuid.uuid4')
    def test_create_task_with_empty_context(
        self, mock_uuid, mock_add_task, task_manager
    ):
