...
from ciris_engine.schemas.services.graph_core import GraphScope, NodeType, GraphNode
from ciris_engine.schemas.handlers.memory_schemas import RecalledNodeInfo, RecallResult
...
def patch_persistence_properly(test_task: Optional[Task] = None) -> Any:
    ...
    with patch('ciris_engine.logic.persistence.update_thought_status', return_value=True) as mock_update_status, \
         patch('ciris_engine.logic.persistence.add_thought') as mock_add_thought, \
         patch('ciris_engine.logic.persistence.add_correlation') as mock_add_correlation, \
         patch('ciris_engine.logic.persistence.get_task_by_id', return_value=test_task) as mock_get_task:
...
class TestRecallHandler:
    ...
    async def test_successful_recall_by_query(
        self, recall_handler: RecallHandler, action_result: ActionSelectionDMAResult, 
        test_thought: Thought, dispatch_context: DispatchContext,
        mock_memory_bus: AsyncMock, test_task: Task, sample_memory_nodes: List[GraphNode]
    ) -> None:
        ...
        with patch_persistence_properly(test_task) as mock_persistence:
            # Execute handler
            follow_up_id = await recall_handler.handle(
    ...
    async def test_limit_enforcement(
        ...
        params = RecallParams(
            query=None,
            node_type=NodeType.TASK_SUMMARY.value,
            node_id=None,
            scope=GraphScope.LOCAL,
            limit=2
        )
    ...
    async def test_audit_trail(
        self, recall_handler: RecallHandler, action_result: ActionSelectionDMAResult,
        test_thought: Thought, dispatch_context: DispatchContext,
        mock_bus_manager: Mock, test_task: Task, sample_memory_nodes: List[GraphNode]
    ) -> None:
    ...
    async def test_service_correlation_tracking(
        self, recall_handler: RecallHandler, action_result: ActionSelectionDMAResult,
        test_thought: Thought, dispatch_context: DispatchContext,
        test_task: Task, sample_memory_nodes: List[GraphNode]
    ) -> None:
