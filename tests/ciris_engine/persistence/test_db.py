...
@patch("ciris_engine.persistence.get_sqlite_db_full_path", new=in_memory_db_path)
def test_initialize_database_and_get_all_tasks():
    db_path = temp_db_file()
    try:
        db.initialize_database(db_path=db_path)
        assert persistence.get_all_tasks(db_path=db_path) == []
    finally:
        os.unlink(db_path)
...
def test_insert_and_get_tasks_by_status():
    ...
    try:
        ...
        with db.get_db_connection(db_path) as conn:
            ...
            conn.commit()
        all_tasks = persistence.get_all_tasks(db_path=db_path)
        assert len(all_tasks) == 2
        active_tasks = persistence.get_tasks_by_status(TaskStatus.ACTIVE, db_path=db_path)
        assert len(active_tasks) == 1
        assert active_tasks[0].task_id == "t1"
        completed_tasks = persistence.get_tasks_by_status(TaskStatus.COMPLETED, db_path=db_path)
...
def test_insert_and_get_thoughts_by_status():
    ...
    try:
        ...
        with db.get_db_connection(db_path) as conn:
            ...
            conn.commit()
        pending = persistence.get_thoughts_by_status(ThoughtStatus.PENDING, db_path=db_path)
        assert len(pending) == 1
        assert pending[0].thought_id == "th1"
        processing = persistence.get_thoughts_by_status(ThoughtStatus.PROCESSING, db_path=db_path)
...
def test_get_tasks_older_than():
    ...
    try:
        ...
        with db.get_db_connection(db_path) as conn:
            ...
            conn.commit()
        older = persistence.get_tasks_older_than("2025-05-28T00:00:00Z", db_path=db_path)
        assert len(older) == 1
...
def test_get_thoughts_older_than():
    ...
    try:
        ...
        with db.get_db_connection(db_path) as conn:
            ...
            conn.commit()
        older = persistence.get_thoughts_older_than("2025-05-28T00:00:00Z", db_path=db_path)
        assert len(older) == 1
