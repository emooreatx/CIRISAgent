...
class TestTelemetryEndpoints:
    ...
    def test_get_reasoning_traces(self, app_with_telemetry, observer_headers, mock_visibility_service, sample_task, sample_thought):
        ...
        # Create a proper mock thought step with all required attributes
        ...
        thought_step_mock.action = "MEMORIZE"  # Provide as string, not Mock
        thought_step_mock.confidence = 0.95  # Provide as float, not Mock
        
        # Configure the trace mock with proper attributes
        trace_mock.thought_steps = [thought_step_mock]
        trace_mock.total_thoughts = 1
        trace_mock.decisions = []  # Empty list to avoid hasattr issues
        trace_mock.actions_taken = ["MEMORIZE"]
        trace_mock.processing_time_ms = 150.0
        trace_mock.max_depth = 3
        trace_mock.outcome = "Task completed successfully"
