...
class TestCIRISSDKEndpoints:
    ...
    async def client(self) -> AsyncGenerator[CIRISClient, None]:
        ...
        async with CIRISClient(base_url="http://localhost:8080", timeout=10.0) as client:  # Reduced timeout for tests
    ...
    async def test_memory_store(self, client: CIRISClient) -> None:
        ...
        node = GraphNode(
            ...
            type="concept",
            scope="local",
            attributes={"test": True, "created_at": datetime.now(timezone.utc).isoformat(), "data": "Test memory node"},
    ...
    async def test_memory_query(self, client: CIRISClient) -> None:
        ...
        nodes = await client.memory.query(query="test", limit=10)
    ...
    async def test_memory_recall(self, client: CIRISClient) -> None:
        ...
        node = GraphNode(
            ...
            type="concept",
            scope="local",
            attributes={"test": True, "created_at": datetime.now(timezone.utc).isoformat()},
            updated_by="test_user",
            updated_at=datetime.now(timezone.utc),
        )
        result = await client.memory.store(node)
    ...
    async def test_audit_search(self, client):
        ...
        results = await client.audit.search(search_text="test", limit=10)
    ...
    async def test_audit_export(self, client):
        ...
        export = await client.audit.export(format="jsonl", start_date=datetime.now(timezone.utc) - timedelta(days=1))
    ...
    async def test_wa_guidance(self, client):
        ...
        guidance = await client.wa.guidance(
            topic="Should I implement this feature?", context="Testing WA guidance endpoint"
        )
