...
def main():
    ...
    debt_parser = subparsers.add_parser(
        "tech-debt",
        help="Analyze technical debt"
    )
    
    # Status command
    status_parser = subparsers.add_parser(
        "status",
        help="Show quick quality status"
    )
    
    # Hotspots command
    hotspots_parser = subparsers.add_parser(
        "hotspots",
        help="List security hotspots"
    )
    ...
    find_parser = subparsers.add_parser(
        "find",
        help="Find file coverage by pattern"
    )
    find_parser.add_argument(
        "pattern",
        help="Pattern to search for in file paths"
    )
