...
class CommandHandler:
    ...
    def quick_status(self) -> None:
        ...
        try:
            ...
            failing = [c for c in qg_status.get('conditions', []) 
                      if c['status'] != 'OK']
            
            if failing:
                print("\nFailing conditions:")
                for cond in failing:
                    print(f"  - {cond['metricKey']}: {cond['actualValue']} "
    ...
    def find_file_coverage(self, path_pattern: str) -> None:
        ...
        try:
            ...
            for file in sorted(matching, key=lambda x: x['coverage']):
                print(f"{file['coverage']:5.1f}% - {file['path']} "
                      f"({file['uncovered_lines']} uncovered lines)")
