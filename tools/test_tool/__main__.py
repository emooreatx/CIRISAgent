...
Examples:
  ...
  python -m tools.test_tool start --no-rebuild
    ...
    start_parser.add_argument("--compose-file", default=DEFAULT_COMPOSE_FILE, help="Docker compose file to use")
    start_parser.add_argument("--no-rebuild", action="store_true", help="Skip rebuilding the container")
    start_parser.add_argument("--parallel", type=int, help="Number of parallel workers")
    start_parser.add_argument(
        "-v", "--verbose", action="count", default=1, help="Increase verbosity (can be used multiple times)"
    )
    ...
    test_parser.add_argument("--no-rebuild", action="store_true", help="Skip rebuilding the container")
    test_parser.add_argument("-v", "--verbose", action="count", default=1, help="Increase verbosity")
    ...
    logs_parser.add_argument("--tail", type=int, default=50, help="Number of lines to show (0 for all)")
    logs_parser.add_argument("--errors", action="store_true", help="Show only errors and failures with context")
