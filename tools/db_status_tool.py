...
class DBStatusTool:
    ...
    def get_overall_status(self) -> Dict[str, Any]:
        ...
        if Path(self.db_path).exists():
            ...
            status["main_db"]["modified"] = datetime.fromtimestamp(
                Path(self.db_path).stat().st_mtime, tz=timezone.utc
            )
    ...
    def get_consolidated_state_analysis(self) -> Dict[str, Any]:
        ...
        with self.get_connection() as conn:
            ...
            analysis["edge_connectivity"]["by_type"] = {
                row["relationship"]: row["count"] for row in cursor
            }
    ...
    def get_available_test_periods(self) -> Dict[str, Any]:
        ...
        with self.get_connection() as conn:
            ...
            for row in unconsolidated:
                periods["unconsolidated"]["hours"].append({
                    "hour": row["hour_slot"],
                    "node_count": row["node_count"]
                })
            ...
            if raw_periods:
                ...
                periods["recommendations"].append({
                    ...
                    "suggested_period": f"Any 6-hour period with raw data"
                })
    ...
    def print_status_report(self):
        ...
        if status["graph_nodes"].get("total"):
            ...
            print("\nNodes by Type:")
            for node_type, count in sorted(status["graph_nodes"]["by_type"].items(), 
        ...
        if status["correlations"].get("total"):
            ...
            if status["correlations"].get("by_type"):
                print("\nBy Type:")
                for corr_type, count in sorted(status["correlations"]["by_type"].items(),
    ...
    def print_audit_report(self):
        ...
        if status["signatures"]:
            ...
            print(f"Signature Coverage: {sigs['signed_entries'] / (sigs['signed_entries'] + sigs['unsigned_entries']) * 100:.1f}%")
            print(f"\nSigning Keys:")
    ...
    def print_consolidated_state_analysis(self):
        ...
        if analysis["data_retention"]:
            ...
            if retention["basic_summaries_old"] > 0:
                print(f"⚠️  Basic summaries older than 7d: {retention['basic_summaries_old']:,} nodes (should be cleaned)")
            else:
                print("✓ No basic summaries older than 7 days")
                
            if retention["daily_summaries_old"] > 0:
                print(f"⚠️  Daily summaries older than 30d: {retention['daily_summaries_old']:,} nodes (should be cleaned)")
            else:
...
Examples:
  ...
  python db_status_tool.py periods      # Available test periods
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=int,
        help="Sample size for verify command (omit for full verification)"
    )
