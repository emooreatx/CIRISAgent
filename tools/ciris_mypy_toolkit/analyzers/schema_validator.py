...
class SchemaValidator:
            ...
            try:
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.ClassDef):
                        ...
                        if any(
                            base.id in ["BaseModel", "BaseSchema", "Enum", "str", "int"]
    ...
    def validate_file(self, file_path: Path) -> Dict[str, Any]:
        ...
        if should_skip_file(str(file_path)):
            return {"file": str(file_path), "total_issues": 0, "issues": [], "compliant": True, "skipped": True}
                ...
                if isinstance(node, ast.ClassDef):
                    ...
                    for class_node in ast.walk(node):
                        if isinstance(class_node, ast.AnnAssign) and isinstance(class_node.target, ast.Name):
                            ...
                            if any(
                                indicator in attr_name.lower()
                                for indicator in ["id", "status", "timestamp", "data", "context", "result"]
                            ):
                                has_data_attrs = True
                                break
