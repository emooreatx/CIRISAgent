...
class HotColdPathAnalyzer:
    ...
    def analyze_module(self, module_path: str) -> Dict[str, Dict]:
        ...
        for node in ast.walk(tree):
            ...
            if isinstance(node, ast.FunctionDef):
                ...
                if any(
                    critical in func_name
                    for critical in ["audit", "security", "auth", "error", "critical", "emergency"]
                ):
    ...
    def _generate_telemetry_recommendations(self, analysis_result: Dict) -> List[str]:
        ...
        important_cold_types = [
            t for t in cold_types if any(critical in t.lower() for critical in ["auth", "audit", "security", "error"])
        ]
