...
from mypy.types import AnyType, Instance, Type, get_proper_type
...
class NoDictAnyPlugin(Plugin):
    ...
    def _check_dict_type(self, typ: Type) -> Type:
        ...
        if isinstance(proper_type, Instance):
            ...
            if proper_type.type.fullname == "builtins.dict" and len(proper_type.args) == 2:
                ...
                if (
                    isinstance(key_proper, Instance)
                    and key_proper.type.fullname == "builtins.str"
                    and isinstance(value_proper, AnyType)
                ):

                    # Report error
                    self.fail("Dict[str, Any] is not allowed. " "Use a specific Pydantic model instead.", proper_type)
