...
class TSDBConsolidationService(BaseGraphService):
    ...
    async def start(self) -> None:
        ...
        logger.info(f"TSDBConsolidationService started - Basic: {self._basic_interval}, Extensive: {self._extensive_interval}, Profound: {self._profound_interval}")
    ...
    async def _extensive_consolidation_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                if wait_seconds > 0:
                    logger.info(f"Next extensive consolidation on Monday {next_run.date()} at 00:00 UTC ({wait_seconds:.0f}s)")
                    await asyncio.sleep(wait_seconds)
    ...
    async def _profound_consolidation_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                if wait_seconds > 0:
                    logger.info(f"Next profound consolidation on {next_run.strftime('%Y-%m-01')} at 00:00 UTC ({wait_seconds:.0f}s)")
                    await asyncio.sleep(wait_seconds)
    ...
    async def _run_consolidation(self) -> None:
        ...
        try:
            ...
            self._edge_manager.cleanup_orphaned_edges()
    ...
    async def _consolidate_period(
        ...
        tsdb_nodes = nodes_by_type.get('tsdb_data', TSDBNodeQueryResult(nodes=[], period_start=period_start, period_end=period_end)).nodes
        ...
        # Audit summary
        audit_nodes = nodes_by_type.get('audit_entry', TSDBNodeQueryResult(nodes=[], period_start=period_start, period_end=period_end)).nodes
        if audit_nodes:
    ...
    async def _create_all_edges(
        ...
        for summary in summaries:
            if summary.type == NodeType.TSDB_SUMMARY:
                ...
                tsdb_nodes = nodes_by_type.get('tsdb_data', TSDBNodeQueryResult(nodes=[], period_start=period_start, period_end=period_start)).nodes
                ...
                audit_nodes = nodes_by_type.get('audit_entry', TSDBNodeQueryResult(nodes=[], period_start=period_start, period_end=period_start)).nodes
        ...
        if all_nodes_in_period:
            ...
            primary_summary = next(
                (s for s in summaries if s.type == NodeType.TSDB_SUMMARY),
    ...
    def is_healthy(self) -> bool:
        ...
        return (
            ...
            (self._consolidation_task is None or not self._consolidation_task.done()) and
            (self._extensive_task is None or not self._extensive_task.done()) and
            (self._profound_task is None or not self._profound_task.done())
        )
    ...
    def get_status(self) -> ServiceStatus:
        ...
        return ServiceStatus(
            ...
            metrics={
                ...
                "last_extensive_consolidation": self._last_extensive_consolidation.timestamp() if self._last_extensive_consolidation else 0.0,
                "last_profound_consolidation": self._last_profound_consolidation.timestamp() if self._last_profound_consolidation else 0.0,
                "basic_task_running": 1.0 if (self._consolidation_task and not self._consolidation_task.done()) else 0.0,
        ...
        try:
            ...
            with get_db_connection() as conn:
                ...
                summary_types = ['tsdb_summary', 'audit_summary', 'trace_summary', 
                                'conversation_summary', 'task_summary']
                    ...
                    for day, day_summaries in summaries_by_day.items():
                            ...
                            if summary_type == 'tsdb_summary':
                                ...
                                for metric, stats in attrs.get('metrics', {}).items():
                                    if metric not in daily_metrics:
                                        daily_metrics[metric] = {'count': 0, 'sum': 0, 'min': float('inf'), 'max': float('-inf')}
                                    ...
                                    if isinstance(stats, dict):
                                        ...
                                        daily_metrics[metric]['min'] = min(daily_metrics[metric]['min'], stats.get('min', float('inf')))
                                        daily_metrics[metric]['max'] = max(daily_metrics[metric]['max'], stats.get('max', float('-inf')))
                                    else:
                        ...
                        if summary_type == 'tsdb_summary':
                            daily_attrs.update({
                                ...
                                "success_rate": 1.0 - (daily_error_count / sum(daily_action_counts.values())) if sum(daily_action_counts.values()) > 0 else 1.0
                            })
