...
class BaseScheduledService(BaseService):
    ...
    async def _on_start(self) -> None:
        ...
        self._logger.info(
            f"{self.service_name}: Started scheduled task with interval {self._run_interval}s"
        )
    ...
    async def _run_loop(self) -> None:
        ...
        while self._started:
            try:
                ...
                self._logger.debug(
                    f"{self.service_name}: Completed scheduled task run #{self._task_run_count}"
                )
                ...
                self._track_error(e)
                self._logger.error(
                    f"{self.service_name}: Error in scheduled task: {e}",
                    exc_info=True
                )
