    ...
    try:
        ...
        for method_name in required_methods:
            if not hasattr(adapter_class, method_name):
                ...
                raise AttributeError(f"Adapter class for mode '{mode}' does not fully implement BaseAdapterProtocol (missing {method_name}).")
        ...
        raise ValueError(f"Could not load 'Adapter' class from mode '{mode}' or it's missing BaseAdapterProtocol methods. Ensure it's defined and implements BaseAdapterProtocol.") from e
