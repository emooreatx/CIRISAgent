...
from ciris_engine.schemas.adapters.core import ChannelInfo, MessageResponse
...
class APIAgentRoutes:
    ...
    def __init__(self, bus_manager: Any, on_message_callback: Optional[Any] = None, service_registry: Optional[Any] = None) -> None:
    ...
    async def _handle_send_message(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            response = MessageResponse(
                ...
                channel_id=channel_id,
                timestamp=datetime.fromisoformat(msg.timestamp) if msg.timestamp else (datetime.fromisoformat(self._get_time_service().now_iso()) if self._get_time_service() else datetime.now(timezone.utc))
            )
    ...
    async def _handle_channel_info(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            channel_info = ChannelInfo(
                ...
                created_at=datetime.fromisoformat(first_message.timestamp) if first_message and first_message.timestamp else (datetime.fromisoformat(self._get_time_service().now_iso()) if self._get_time_service() else datetime.now(timezone.utc)),
                last_activity=datetime.fromisoformat(last_message.timestamp) if last_message and last_message.timestamp else (datetime.fromisoformat(self._get_time_service().now_iso()) if self._get_time_service() else datetime.now(timezone.utc)),
