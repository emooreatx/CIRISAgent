...
class QueryRequest(BaseModel):
    ...
    def validate_query_params(self) -> "QueryRequest":
        ...
        if not any([self.node_id, self.type, self.query, self.since, self.related_to]):
...
async def store_memory(
    request: Request, body: StoreRequest, auth: AuthContext = Depends(require_admin)
...
async def query_memory(
    request: Request, body: QueryRequest, auth: AuthContext = Depends(require_observer)
    ...
    try:
        ...
        # Type-based query
            ...
            all_nodes = await memory_service.recall(
                MemoryQuery(
...
async def forget_memory(
    ...
    try:
        # First get the node
        query = MemoryQuery(node_id=node_id, scope=GraphScope.LOCAL, type=None, include_edges=False, depth=1)
        nodes = await memory_service.recall(query)
...
async def get_timeline(
    ...
    try:
        ...
        from ciris_engine.schemas.services.graph_core import NodeType as NodeTypeEnum
        ...
        try:
            with get_db_connection() as conn:
                ...
                for day_offset in range(days_in_range):
                    ...
                    query_parts = [SQL_SELECT_NODES, SQL_FROM_NODES, SQL_WHERE_TIME_RANGE, SQL_EXCLUDE_METRICS]
            ...
            all_nodes = await memory_service.search(
                "",
                filters=MemorySearchFilter(
                    scope=scope.value if scope else None, node_type=type.value if type else None, limit=1000
                ),
            )
        ...
        response = TimelineResponse(
            memories=returned_nodes, edges=edges, buckets=buckets, start_time=start_time, end_time=now, total=len(nodes)
        )
...
async def get_memory(
    ...
    try:
        query = MemoryQuery(
            node_id=node_id, scope=GraphScope.LOCAL, type=None, include_edges=include_edges, depth=depth
        )
    ...
    try:
        ...
        if hours:
            ...
            if layout == "timeline":
                ...
                if limit > effective_limit:
                    logger.info(
                        f"Capping timeline visualization from {limit} to {effective_limit} nodes for performance"
                    )
                import json

                try:
                    logger.info(
                        f"Timeline visualization: Querying database for {hours} hours with limit {effective_limit}"
                    )
                    with get_db_connection() as conn:
                        cursor = conn.cursor()
                        ...
                        if hours <= 48:
                            ...
                            logger.info(
                                f"Timeline: {num_buckets} buckets ({bucket_hours}h each), {nodes_per_bucket} nodes per bucket"
                            )
                        ...
                        logger.info(
                            f"Timeline: Collected {len(all_db_nodes)} nodes, using {len(nodes)} for visualization"
                        )
                    ...
                    memory_query = MemoryQuery(
                        node_id="*", scope=scope or GraphScope.LOCAL, type=node_type, include_edges=False, depth=1
                    )
            ...
            if layout != "timeline" and nodes == []:
                ...
                if not include_metrics and node_type != NodeType.TSDB_DATA:
                    all_nodes = [
                        n for n in all_nodes if not (n.type == NodeType.TSDB_DATA and n.id.startswith("metric_"))
                    ]
        ...
        for node in nodes:
            ...
            node_attrs = {
                ...
                "created_at": (
                    (
                        node.attributes.get("created_at")
                        or node.attributes.get("timestamp")
                        or str(getattr(node, "updated_at", ""))
                    )
                    if isinstance(node.attributes, dict)
                    else str(getattr(node.attributes, "created_at", getattr(node, "updated_at", "")))
                ),
        ...
        raise HTTPException(
            status_code=503, detail="Graph visualization requires networkx. Please install: pip install networkx"
        )
    except Exception as e:
...
def _hierarchy_pos(
    G: "nx.DiGraph", root: str, width: float = 1.0, vert_gap: float = 0.2, vert_loc: float = 0, xcenter: float = 0.5
...
def _calculate_timeline_layout(
    G: "nx.DiGraph", nodes: List[GraphNode], width: int, height: int, hours: Optional[int] = None
) -> Dict[str, Tuple[float, float]]:
...
def _generate_svg(
    ...
    for i, (node_type, label) in enumerate(type_colors):
        ...
        svg_parts.append(f'<circle cx="30" cy="{y_offset}" r="8" fill="{color}" stroke="#333" stroke-width="1"/>')
        svg_parts.append(
...
async def create_edge(
    request: Request, body: CreateEdgeRequest, auth: AuthContext = Depends(require_admin)
) -> SuccessResponse[MemoryOpResult]:
    """
