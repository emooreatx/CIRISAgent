...
class SpeakContext(BaseActionContext):
    ...
    message_type: Literal["response", "notification", "error"] = Field(
        "response", description="Type of message"
    )
...
class ObserveContext(BaseActionContext):
    ...
    observation_type: Literal["channel", "user", "system"] = Field(
        "channel", description="What to observe"
    )
...
class MemorizeContext(BaseActionContext):
    ...
    memory_scope: Literal["task", "conversation", "global"] = Field(
        "task", description="Scope of memory"
    )
...
class RecallContext(BaseActionContext):
    ...
    memory_scope: Literal["task", "conversation", "global"] = Field(
        "task", description="Scope to search"
    )
...
class TaskCompleteContext(BaseActionContext):
    ...
    completion_status: Literal["success", "partial", "failure"] = Field(
        "success", description="Completion status"
    )
