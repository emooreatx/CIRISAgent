...
class CoreProfile(BaseModel):
    ...
    domain_specific_knowledge: Dict[str, str] = Field(
        ...
        description="Domain expertise mappings"
    )
    ...
    dsdma_prompt_template: Optional[str] = Field(
        None,
        description="Custom DSDMA prompt template"
    )
    csdma_overrides: Dict[str, str] = Field(
        default_factory=dict,
        description="Common sense overrides"
    )
    ...
    last_shutdown_memory: Optional[str] = Field(
        None,
        description="Memory from last shutdown"
    )
...
class IdentityMetadata(BaseModel):
    ...
    lineage_trace: List[str] = Field(
        default_factory=list,
        description="Identity evolution lineage"
    )
...
class CapabilityDefinition(BaseModel):
    ...
    # Constraints
    usage_limits: Optional[Dict[str, int]] = Field(
        None,
        description="Usage limits per time period"
    )
    restricted_contexts: List[str] = Field(
        default_factory=list,
        description="Contexts where restricted"
    )

    # Dependencies
    depends_on: List[str] = Field(
        default_factory=list,
        description="Other required capabilities"
    )
...
class AgentIdentityRoot(BaseModel):
    ...
    authorization_scope: str = Field(
        "standard",
        description="Authorization scope: limited, standard, elevated, full"
    )

    # Relationships
    parent_agent_id: Optional[str] = Field(
        None,
        description="Parent agent if spawned"
    )
