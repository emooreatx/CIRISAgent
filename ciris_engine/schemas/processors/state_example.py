    ...
    history = StateHistory(
        transitions=[
            StateTransitionRecord(
                ...
                to_state=AgentState.SHUTDOWN.value
            ),
            StateTransitionRecord(
                timestamp="2025-01-01T08:00:10Z",
                from_state=AgentState.SHUTDOWN.value,
                to_state=AgentState.WAKEUP.value
            ),
            StateTransitionRecord(
        ...
        current_state_metadata=StateMetadata(
            entered_at="2025-01-01T08:05:00Z",
            metrics={"thoughts_processed": 42, "tasks_completed": 3}
        )
    )
...
def example_state_validation() -> None:
    ...
    validation = StateTransitionValidation(
        ...
        conditions=[
            StateCondition(
                name="shutdown_allowed",
                description="Shutdown transitions are allowed from any state",
                met=True
            ),
            StateCondition(
                name="dream_complete",
