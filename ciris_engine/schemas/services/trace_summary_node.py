...
class TraceSummaryNode(TypedGraphNode):
    ...
    tasks_by_status: Dict[str, int] = Field(
        ...
        description="Task count by final status"
    )
    unique_task_ids: Set[str] = Field(
        default_factory=set,
    ...
    thoughts_by_type: Dict[str, int] = Field(
        default_factory=dict,
        description="Thought count by type"
    )
    ...
    component_calls: Dict[str, int] = Field(
        default_factory=dict,
        description="Calls to each component type (processor, dma, guardrail, handler)"
    )
    component_failures: Dict[str, int] = Field(
        default_factory=dict,
        description="Failures by component type"
    )
    component_latency_ms: Dict[str, Dict[str, float]] = Field(
        default_factory=dict,
        description="Latency stats by component: {component: {avg, p95, p99}}"
    )
    
    # Processing patterns
    dma_decisions: Dict[str, int] = Field(
        default_factory=dict,
        description="Count of decisions by DMA type"
    )
    guardrail_violations: Dict[str, int] = Field(
        default_factory=dict,
        description="Count of violations by guardrail type"
    )
    handler_actions: Dict[str, int] = Field(
        default_factory=dict,
        description="Count of actions by handler type"
    )
    ...
    def from_graph_node(cls, node: GraphNode) -> 'TraceSummaryNode':
        ...
        return cls(
            ...
            consolidation_timestamp=cls._deserialize_datetime(
                attrs.get("consolidation_timestamp")
